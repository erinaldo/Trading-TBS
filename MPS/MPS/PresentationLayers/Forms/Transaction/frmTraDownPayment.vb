...
#End Region
    ...
    Private Sub prvFillCombo()
        Try
            Dim dtData As DataTable = BL.StatusModules.ListDataByModulesID(IIf(intDPType = VO.DownPayment.Type.Sales, VO.Modules.Values.TransactionSalesDownPayment, VO.Modules.Values.TransactionReceiveDownPayment))
            Dim dr As DataRow
            dr = dtData.NewRow
            With dr
                .BeginEdit()
    ...
    Private Sub prvQuery()
        ...
        pgMain.Value = 30
        Try
            grdMain.DataSource = BL.DownPayment.ListData(intCompanyID, MPSLib.UI.usUserApp.ProgramID, dtpDateFrom.Value.Date, dtpDateTo.Value.Date, cboStatus.SelectedValue, intDPType)
            prvSumGrid()
    ...
    Private Sub prvUserAccess()
        With ToolBar.Buttons
            .Item(cNew).Visible = BL.UserAccess.IsCanAccess(MPSLib.UI.usUserApp.UserID, MPSLib.UI.usUserApp.ProgramID, IIf(intDPType = VO.DownPayment.Type.Sales, VO.Modules.Values.TransactionSalesDownPayment, VO.Modules.Values.TransactionReceiveDownPayment), VO.Access.Values.NewAccess)
            .Item(cDelete).Visible = BL.UserAccess.IsCanAccess(MPSLib.UI.usUserApp.UserID, MPSLib.UI.usUserApp.ProgramID, IIf(intDPType = VO.DownPayment.Type.Sales, VO.Modules.Values.TransactionSalesDownPayment, VO.Modules.Values.TransactionReceiveDownPayment), VO.Access.Values.DeleteAccess)
        End With
    End Sub

Public Class frmMstUOM
    ...
    Private Sub prvQuery()
        Try
            grdMain.DataSource = BL.UOM.ListData()
    ...
    Private Sub prvGet()
        ...
        If grdView.GetRowCellValue(intPos, "IDStatus") = VO.Status.Values.InActive Then
            UI.usForm.frmMessageBox("Tidak dapat pilih satuan " & grdView.GetRowCellValue(intPos, "Name") & ". Dikarenakan barang tersebut sudah tidak aktif")
            Exit Sub
        Else
    ...
    Private Sub prvNew()
        Dim frmDetail As New frmMstUOMDet
        With frmDetail
            .pubIsNew = True
    ...
    Private Sub prvDelete()
        ...
        Try
            BL.UOM.DeleteData(grdView.GetRowCellValue(intPos, "ID"))
    ...
    Private Sub prvUserAccess()
        With ToolBar.Buttons
            .Item(cNew).Visible = BL.UserAccess.IsCanAccess(MPSLib.UI.usUserApp.UserID, MPSLib.UI.usUserApp.ProgramID, VO.Modules.Values.MasterUOM, VO.Access.Values.NewAccess)
            .Item(cDelete).Visible = BL.UserAccess.IsCanAccess(MPSLib.UI.usUserApp.UserID, MPSLib.UI.usUserApp.ProgramID, VO.Modules.Values.MasterUOM, VO.Access.Values.DeleteAccess)
        End With
    End Sub

#Region "Form Handle"

    Private Sub frmMstUOM_KeyDown(sender As Object, e As KeyEventArgs) Handles MyBase.KeyDown
        If e.KeyCode = Keys.Escape Then
            If UI.usForm.frmAskQuestion("Tutup form ini?") Then Me.Close()
        End If
    End Sub

    Private Sub frmMstUOM_Load(sender As Object, e As EventArgs) Handles MyBase.Load
